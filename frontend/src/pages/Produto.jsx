...
function Produto() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/produtos/${codigo}`)
      .then(r => {
        setProduto(r.data)
        return Promise.all([
          axios.get(`${API}/api/produtos?categoria=${encodeURIComponent(r.data.categoria)}&limite=8`),
          axios.get(`${API}/api/produtos/${codigo}/imagens`),
        ])
      })
      .then(([relRes, imgRes]) => {
        setRelacionados(relRes.data.produtos.filter(p => p.codigo !== parseInt(codigo)).slice(0, 4))
  ...
  if (!produto) return (
    <div className="text-center py-16 text-gray-400">
      ...
      <Link to="/catalogo" className="text-sky-500 mt-4 inline-block">← Voltar ao catálogo</Link>
    </div>
  )

  const srcPrincipal = imagens.length > 0
    ? imagens[imgAtiva]
    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <nav className="text-xs text-gray-400 mb-8 flex items-center gap-2 flex-wrap">
          ...
          <span>›</span>
          <Link to="/catalogo" className="hover:text-sky-500">PRODUTOS</Link>
          <span>›</span>
          <Link to={`/catalogo?categoria=${encodeURIComponent(produto.categoria)}`}
            className="hover:text-sky-500 uppercase">{produto.categoria}</Link>
          <span>›</span>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:w-[480px] flex-shrink-0">
            <div className="flex items-center justify-center h-[480px] mb-4">
              <img
                ...
                onError={e => { e.target.src = 'https://placehold.co/480x480?text=Sem+foto' }}
              />
            </div>
            {imagens.length > 1 && (
          ...
          <div className="flex-1">
            <p className="text-gray-500 text-sm mb-1">{String(produto.codigo).padStart(5, '0')}</p>
            ...
            <div className="space-y-2 mb-6 text-sm">
              ...
              {produto.largura && (
                <p>
                  <span className="font-semibold text-gray-700">Largura : </span>
                  <span className="text-gray-500 italic">{produto.largura}</span>
                </p>
              )}
              {produto.medidas && (
                <p>
                  <span className="font-semibold text-gray-700">Medidas aproximadas para gravação </span>
                  <span className="text-gray-500 italic">{produto.medidas}</span>
                </p>
              )}
              {produto.peso && (
                <p>
                  <span className="font-semibold text-gray-700">Peso aproximado </span>
            ...
            <div className="mb-8">
              <p className="text-sm font-semibold text-gray-700 mb-2">Veja mais brindes</p>
              <Link to={`/catalogo?categoria=${encodeURIComponent(produto.categoria)}`}
              ...
              {relacionados.map(p => {
                ...
                return (
                  <Link key={p.codigo} to={`/catalogo/${p.codigo}`} className="group text-center">
                    ...
                    <p className="text-xs text-gray-400 font-mono">{String(p.codigo).padStart(5, '0')}</p>
                    <p className="text-sm text-gray-600 mt-1 line-clamp-2">{p.nome}</p>
                  </Link>
                )
              })}
            </div>
